              ...
              <Card key={class_.id} className="">
                ...
                <CardContent>
                  <CardDescription>
                    ...
                    <span className=" font-bold"> {class_.teachingMode}</span>{" "}
                    demo class for {class_.subject} at{" "}
                    {moment(class_.startTime)
                      .local()
                      .format("YYYY-MM-DD HH:mm:ss")
                      .slice(11, 16)}{" "}
                    on {returnClassDate(class_.startTime)}
                  </CardDescription>
                </CardContent>
                ...
                <CardContent>
                  <CardDescription>
                    <span className=" font-bold">
                      You completed a {class_.type} class{" "}
                    </span>
                    for {class_.subject} from{" "}
                    {moment(class_.startTime)
                      .local()
                      .format("YYYY-MM-DD HH:mm:ss")
                      .slice(11, 16)}{" "}
                    to{" "}
                    {moment(class_.endTime)
                      .local()
                      .format("YYYY-MM-DD HH:mm:ss")
                      .slice(11, 16)}{" "}
                    on {returnClassDate(class_.startTime)}
                ...
                <CardFooter className="flex justify-end">
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button size="sm">View class details</Button>
                    </DialogTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2">
            ...
            {expired_not_completed_classes.map((class_) => (
              <Card key={class_.id} className="">
                ...
                <CardFooter className="flex justify-end">
                  <Button size="sm">reassign class</Button>
                </CardFooter>
              </Card>
            ))}
          </div>
